...
import os # 환경 변수 사용을 위해 os 모듈 import
import pymysql.cursors
import pandas as pd
from surprise import SVD, Dataset, Reader
from surprise.model_selection import train_test_split # 학습/테스트 분할용 (필요시)
from surprise import Prediction
import numpy as np # 아이템 ID 리스트를 위한 numpy

app = Flask(__name__)

# --- MariDB 설정 (⚠️ 실제 정보로 변경) ---
# [개선] 보안을 위해 하드코딩된 설정 대신 환경 변수에서 불러오도록 변경
DB_CONFIG = {
    'host': os.getenv('DB_HOST', 'localhost'),
    'user': os.getenv('DB_USER', 'your_mariadb_username'),
    'password': os.getenv('DB_PASSWORD', 'your_mariadb_password'),
...
# --- 추천 시스템 설정 값 ---
MIN_INTERACTIONS_FOR_ML = 3    # ML 추천을 위한 최소 상호작용 수 (Spring Boot의 application.properties와 동기화)
POPULAR_ITEMS_LIMIT = 5        # 인기 아이템 반환 개수 (Spring Boot의 application.properties와 동기화)
...
# --- 모델 로딩 및 학습 함수 ---
# Flask 앱이 시작될 때 이 함수를 한 번 호출하여 모델을 메모리에 로드하고 학습합니다.
def load_and_train_model():
    ...
    try:
        ...
        sql_interactions = """
            ...
            WHERE interaction_type IN ('VIEW', 'CLICK', 'LIKE', 'PURCHASE', 'ADD_TO_CART', 'COMMENT')
            """
        ...
        model = SVD(random_state=42, n_epochs=20, n_factors=100) # SVD 모델 파라미터 (조정 가능)
        model.fit(trainset)
...
def get_recommendations_logic(user_id):
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            if interaction_count < MIN_INTERACTIONS_FOR_ML or global_recommendation_model is None:
                # 상호작용이 적거나 모델이 로드되지 않은 경우 -> 인기 기반 추천 (레거시 로직)
                ...
                # 상호작용이 충분하고 모델이 로드된 경우 -> ML 모델 기반 추천
...
def recommend():
    try:
        # [개선] 표준 JSON 객체 형식({"userId": "some-id"})으로 요청을 받도록 수정
        data = request.get_json()
        if not data or 'userId' not in data:
            return jsonify({"error": "userId is required in the JSON body"}), 400

        user_id = data['userId'] # userId를 문자열로 받음

        # [개선] Java 백엔드와 일관성을 위해 userId를 문자열로 처리
        print(f"[Flask Server] Received recommendation request for userId: {user_id}")
        
        # 통합 추천 로직 호출 (하이브리드 처리 포함)
        recommended_item_ids = get_recommendations_logic(user_id)
        
        # Surprise 라이브러리가 UUID를 처리하지 못할 수 있으므로,
        # 추천된 ID가 UUID 객체라면 문자열로 변환하여 반환
...
# 실제 운영 환경에서는 Flask 앱을 실행하는 워커(worker)마다 이 코드가 실행될 수 있음을 유의
with app.app_context():
    load_and_train_model()

if __name__ == '__main__':
    # 디버그 모드를 켜면 코드 변경 시 자동 재로드되어 편리 (운영에선 비활성화)
